...
import MasonryGrid from '../components/platform/MasonryGrid'
  ...
  return (
    <div className="section-filter-container">
      ...
      {quickFilters.length > 0 && (
        <div className="section-quick-filters">
          {quickFilters.map(filter => (
            <button
              key={filter}
              className={`quick-filter-pill ${searchQuery.toLowerCase() === filter.toLowerCase() ? 'active' : ''}`}
              onClick={() => onSearchChange(searchQuery.toLowerCase() === filter.toLowerCase() ? '' : filter)}
            >
              {filter}
            </button>
          ))}
...
export default function Home({ data }) {
  useScrollReveal([]) // Run once only — prevents flicker on data re-load
  ...
  const allThumbnails = useMemo(() => workData.filter(i => {
    ...
    return t === 'thumbnail' || t === 'slider' || t === 'poster' || t === 'rp'
  }), [workData])
  ...
  // Only show featured items on the main page
  const featuredThumbnails = useMemo(() => allThumbnails.filter(t => t.featured === true), [allThumbnails])
  const featuredVideos = useMemo(() => allVideos.filter(v => v.featured === true), [allVideos])
  const featuredShorts = useMemo(() => allShorts.filter(s => s.featured === true), [allShorts])

  const filteredThumbnails = useMemo(() => filterItems(featuredThumbnails, thumbSearch, thumbSort), [featuredThumbnails, thumbSearch, thumbSort])
  const filteredVideos = useMemo(() => filterItems(featuredVideos, videoSearch, videoSort), [featuredVideos, videoSearch, videoSort])
  const filteredShorts = useMemo(() => filterItems(featuredShorts, shortSearch, shortSort), [featuredShorts, shortSearch, shortSort])
      ...
      <section id="thumbnails" className="content-section">
        <div className="content-section-inner">
          ...
          <SectionFilterBar
            ...
            hideViewsSort={true}
            quickFilters={['BGMI', 'Valorant', 'Minecraft', 'Vlog', 'Gaming']}
          />
            ...
            {featuredVideos.length === 0 ? (
              ...
              <>
                <SectionFilterBar
                  ...
                  quickFilters={['Montage', 'Gameplay', 'IRL', 'Documentary']}
                />
                {filteredVideos.length === 0 ? (
                  <div className="empty-state"><div className="empty-state-icon">🎬</div><p>No featured videos match your filters.</p></div>
                ) : (
                  <>
                    <MasonryGrid 
                      items={videosDisplay}
        ...
        <section id="shorts" className="content-section">
          <div className="content-section-inner">
            ...
            {featuredShorts.length === 0 ? (
              <div className="empty-state">
                <div className="empty-state-icon">📱</div>
                <p>No featured short available.</p>
              </div>
            ) : (
              <>
                <SectionFilterBar
                  ...
                  quickFilters={['Funny', 'Highlights', 'Tips']}
                />
                {filteredShorts.length === 0 ? (
                  <div className="empty-state"><div className="empty-state-icon">📱</div><p>No featured shorts match your filters.</p></div>
                ) : (
                  <>
                    <MasonryGrid 
                      items={shortsDisplay}
        ...
        <div className="image-modal-overlay" onClick={() => setModalProject(null)}>
          ...
          <div className="image-modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="image-modal-main">
              <div className="image-modal-img-wrap" style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                {(() => {
                  const ytid = (modalProject.type === 'video' || modalProject.type === 'short') ? extractVideoId(modalProject.link) : null;
                  if (ytid) {
                    const isShort = modalProject.type === 'short' || modalProject.aspectRatio === '9:16';
                    return (
                      <iframe 
                        ...
                        style={{ 
                          ...
                          maxHeight: '80vh',
                          maxWidth: isShort ? '45vh' : '100%'
                        }}
                      ></iframe>
                    );
                  }
                  return <img src={thumbUrl(modalProject)} alt={modalProject.title} className="image-modal-img" />;
                })()}
              </div>
            </div>
          </div>
