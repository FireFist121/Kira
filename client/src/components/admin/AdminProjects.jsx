...
export default function AdminProjects({ category, editId, onEditClear }) {
  ...
  const tableColCount = isThumbnail ? 5 : isVideo ? 8 : isPoster ? 5 : 6
  ...
  const load = async () => {
    try {
      ...
      const filtered = allData.filter(p => {
        ...
        if (isThumbnail) return t === 'thumbnail' || t === 'slider' || t === 'poster'
  ...
  const defaultForm = () => {
    return { title: '', type: category, tag: '', featured: false, thumbnail: '', client: '', link: '', views: '', duration: '', beforeImage: '', afterImage: '', aspectRatio: '16:9' }
  }
