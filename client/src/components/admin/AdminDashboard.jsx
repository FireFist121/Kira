...
export default function AdminDashboard({ onLogout }) {
  ...
  return (
    <div className={`dash-root ${desktopMenuClosed ? 'desktop-closed' : ''}`}>
      ...
      <div className="dash-mobile-header">
        ...
        <button className="dash-hamburger" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} aria-label="Toggle sidebar">
          <span />
          <span />
          <span />
        </button>
      ...
      {/* ── OVERLAY ── */}
      <div
        className={`dash-overlay${mobileMenuOpen ? ' open' : ''}`}
        onClick={() => setMobileMenuOpen(false)}
      />

      {/* ── SIDEBAR ── */}
      <aside className={`dash-sidebar${mobileMenuOpen ? ' open' : ''}`}>
        <div className="dash-logo-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <a href="/" className="dash-logo-link">
            <span className="dash-logo">KIR<span className="dash-logo-accent">4</span></span>
          </a>
          <button className="dash-collapse-btn" onClick={() => setDesktopMenuClosed(true)}>
      ...
      <main className="dash-main">
        {desktopMenuClosed && (
          <button className="dash-expand-btn" onClick={() => setDesktopMenuClosed(false)}>
