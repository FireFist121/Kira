      ...
      {isSettings && (
        <>
          <div className="admin-glass-form-card">
            ...
            <p style={{ color: '#666', fontSize: '12px', marginBottom: '20px', marginTop: '-8px' }}>These numbers appear on both the Home hero and About section.</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
              <div className="admin-form-group">
                <label className="admin-label">Projects</label>
                <input className="admin-input" placeholder="e.g. 150+" value={stats.projects} onChange={e => setStats({ ...stats, projects: e.target.value })} />
              </div>
              <div className="admin-form-group">
                <label className="admin-label">Clients</label>
                <input className="admin-input" placeholder="e.g. 40+" value={stats.clients} onChange={e => setStats({ ...stats, clients: e.target.value })} />
              </div>
              <div className="admin-form-group">
                <label className="admin-label">Years</label>
                <input className="admin-input" placeholder="e.g. 3+" value={stats.years} onChange={e => setStats({ ...stats, years: e.target.value })} />
              </div>
            </div>
            <button className="dash-btn-fetch" onClick={handleStatsSave}>Save Stats</button>
          </div>

          <div className="admin-glass-form-card" style={{ marginTop: '24px' }}>
            <h3 className="admin-glass-form-title">Update About Me</h3>
            <p style={{ color: '#666', fontSize: '12px', marginBottom: '20px', marginTop: '-8px' }}>This content appears in the About section of the portfolio.</p>
            <div className="admin-form-group">
              <label className="admin-label">Tagline (Hero subtitle)</label>
              <input
                className="admin-input"
                placeholder="e.g. Thumbnail Designer × Video Editor."
                value={content.tagline}
                onChange={e => setContent({ ...content, tagline: e.target.value })}
              />
            </div>
            <div className="admin-form-group">
              <label className="admin-label">About Me — Paragraph 1</label>
              <textarea
                className="admin-input"
                rows={4}
                style={{ resize: 'vertical', lineHeight: '1.6' }}
                placeholder="I'm KIR4 — a thumbnail designer & video editor..."
                value={content.bio1}
                onChange={e => setContent({ ...content, bio1: e.target.value })}
              />
            </div>
            <div className="admin-form-group">
              <label className="admin-label">About Me — Paragraph 2</label>
              <textarea
                className="admin-input"
                rows={4}
                style={{ resize: 'vertical', lineHeight: '1.6' }}
                placeholder="With 3+ years in the game..."
                value={content.bio2}
            ...
            <div className="admin-form-group">
              ...
              <input
                className="admin-input"
                placeholder="Photoshop, Premiere Pro, After Effects..."
                value={content.skills}
                onChange={e => setContent({ ...content, skills: e.target.value })}
              />
            </div>
            <button className="dash-btn-fetch" onClick={handleContentSave}>Save About Me</button>
          </div>
        </>
      )}
